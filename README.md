...
管理機能は**アクティビティ内**で完結します。代表・副代表・部門長・開発者でアクセスすると、ナビに **🛠 承認** ボタンが表示され、承認・管理画面に切り替わります。一般メンバーには表示されません。別ログインや OAuth の追加設定は不要です（埋め込み SDK のトークンで認証）。
...
- 必要な設定は環境変数 `ADMIN_SUPER_DISCORD_IDS` のみです。
...
> YAML（`mission_data/`）+ `npm run sync-missions` での一括投入も引き続き使えます。大量の初期投入は YAML、日々の運用はアクティビティ内の管理画面、と使い分けられます。Supabase → **Table Editor** での直接編集も可能です。
...
| ミッション内容 | アクティビティ内の管理画面（[管理機能](#管理機能権限を持つ人だけ)）または `mission_data/*.yaml`（[ミッション管理](#ミッション管理)） |
