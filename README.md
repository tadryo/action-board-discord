...
| `ADMIN_SESSION_SECRET` | **必須** | 管理画面を使うなら必要 | `/admin` のセッション署名鍵（秘匿）。`openssl rand -hex 32` で生成 |
| `ADMIN_SUPER_DISCORD_IDS` | **必須** | 〃 | 代表・副代表の Discord ユーザー ID（カンマ区切り）。初回ログインで super として登録される |
...
アプリとは**別ページ**として `/admin` に管理画面があります。Discord OAuth でログインし、権限に応じた操作ができます（Supabase ログインは不要）。
...
- 代表・副代表は `ADMIN_SUPER_DISCORD_IDS`（環境変数）に Discord ユーザー ID を登録すると、初回ログイン時に `super` として登録されます。
- 部門長などは `super`/`developer` が管理画面の「メンバー権限」から任命します。
- 設定: Discord Developer Portal → OAuth2 → Redirects に `https://<本番ドメイン>/admin/callback` を追加し、`ADMIN_SESSION_SECRET` を設定してください。

### ミッションの作成・編集

- **「みんなでやろう」（固定ミッション）**: `super`/`developer` が `/admin` の「ミッション管理」で作成・編集・非表示にできます。
- **「チームタスク」（部門タスク）**: メンバーが Discord アクティビティ内の「＋ タスクを提案」から提案 → 部門長が `/admin` の「提案承認」で承認するとミッション化されます。却下時は理由の詳述が必須で、記録として保存されます（公開はされませんが開示請求に対応可能）。
