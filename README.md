...
Supabase ダッシュボードの **SQL Editor** で `supabase/migrations/` 配下のファイルを番号順に実行します。

```
20260525000001_initial_schema.sql
20260525000002_atomic_achievement.sql
20260525000003_fix_achievement_lock.sql
20260525000004_missions_add_icon_featured.sql
20260529000001_app_settings.sql
20260529000002_categories_group_key.sql
```
...
3. **URL Mappings**: `/` → Railway デプロイ後の URL（後から設定可）
4. サーバーへの導入: Installation の Install Link（スコープに `applications.commands` を含める）
...
cloudflared でトンネルを張り、その URL を Discord Developer Portal の **URL Mappings** に登録します:
